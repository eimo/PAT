...
$(document).on('pageinit', '#home',  function(){
	...
	$("#table-number-form").on('submit', function(e){
		...
		if(PARTNER.partner.demoMode){ //if demo, simulate
			...
			var bind_url = LOAD['req_tableJoinfriend'].replace('{table_code}', table_num);
			...
			$.ajax(bind_url, {dataType: PARTNER.partner.req_dataType}).success(function(data){
				...
				if(data.meta.status!='200'){
					...
					//重设leave friend参数
					LOAD['req_leaveFriend'] = LOAD['req_leaveFriend'].replace('{table_code}', table_num);
					
					$('#leave_friend-button').show();
					if(typeof data.table !== 'undefined'){
						INVOICE = data.table;
						show_bill(INVOICE);
...
$(document).on('pageinit', '#bill',  function(){
	...
	$('#leave_friend-button').on(click_event, function(){
		
		$.ajax(LOAD['req_leaveFriend'], {
			timeout:15000,
			crossDomain: true,
			dataType: PARTNER.partner.req_dataType
				...
				if(jqXHR.responseText !== undefined){
					...
					alert(errorThrown+" | Sorry leave friend operation is failed. Please try again later.");
				}
				$.mobile.loading('hide');
			});
...
$(document).on('pageinit', '#split',  function(){
	...
	$('#split-slider').change(function(){
		...
		var my_amount = (parseFloat($(this).val()) / 100) * (INVOICE.total-INVOICE.other_tender);
	...
	$("#split-slider").on("slidestop", function (event) {
		...
		var my_amount = (parseFloat($(this).val()) / 100) * (INVOICE.total-INVOICE.other_tender);
		var already_paid = parseFloat(INVOICE.PaymentByOtherMeans);
		var grand_total = INVOICE.total-INVOICE.other_tender;
		var remains = grand_total - my_amount;
		
		CHART.series[0].data[0].update( y = my_amount, name = "new name");
		CHART.series[0].data[1].update(y = remains);

		SPLIT_ARRAY['sub_total'] = SPLIT_ARRAY['grand_total'] = SPLIT_AMOUNT = SPLIT = my_amount;
...
$(document).on('pageshow', '#split',  function(){
	...
	if(!SPLIT){
		SPLIT_AMOUNT = SPLIT = (INVOICE.total-INVOICE.other_tender - INVOICE.PaymentByOtherMeans)  / 2;
    ...
    start_up_data[1] = {name: 'Remains',  y: INVOICE.total-INVOICE.other_tender - SPLIT - INVOICE.PaymentByOtherMeans, color: '#d0d0d0'};
	...
	SPLIT_ARRAY['sub_total'] =  SPLIT.toFixed(2);
	
	SPLIT_ARRAY['tax_total'] =  INVOICE.tax;
	SPLIT = SPLIT_ARRAY['grand_total'] =  parseFloat(SPLIT_ARRAY['sub_total'])+parseFloat(SPLIT_ARRAY['tax_total']);

    var userPhoto = INVOICE.customers.customer_photo_url;
	...
	$('#container').highcharts({
        ...
        title: {
            text: 'Bill Total<br/>'+format_cash(INVOICE.total-INVOICE.other_tender),
            align: 'center',
            style: {
                ...
                dataLabels: {
                    ...
                    formatter: function(){
                    	...
                    	if(this.point.name == 'You'){
                    		var name = '<img src="'+userPhoto+'" width="50" style="border:1px solid #d0d0d0; border-radius:25px;" />';
                    	}else{
	...
	$('.tip-button').on(click_event, function(){
		...
		if(!SPLIT){		
			if(MERCHANT.merchant.calculate_tip_including_tax){
				var tip_base = INVOICE.total - INVOICE.other_tender - INVOICE.PaymentByOtherMeans;
			}else{
				var tip_base = INVOICE.sub_total - INVOICE.other_tender - INVOICE.PaymentByOtherMeans;
			}

			var tip_total = parseFloat((tip_base * (tip_percent/100)).toFixed(2));

			var new_grand_total = parseFloat(tip_total) + parseFloat(INVOICE.total) - parseFloat(INVOICE.other_tender) - parseFloat(INVOICE.PaymentByOtherMeans);
		}else{
			var tip_base = SPLIT;
			var tip_total = parseFloat((tip_base * (tip_percent/100)).toFixed(2));
	...
	$('#pay-button').on('click', function(){
		...
		if(!SPLIT){ // use the full table payload
			...
			var payment_payload = {
                ...
                'payment[discount]' : parseFloat(INVOICE.discount).toFixed(2),
				'payment[tax]' : parseFloat(INVOICE.tax_total).toFixed(2),
				'payment[total]' : parseFloat(INVOICE.total - INVOICE.other_tender).toFixed(2),
...
function show_bill(invoice){
	...
	if(typeof INVOICE.PaymentByOtherMeans != "undefined"){
		paid_total =parseFloat(INVOICE.PaymentByOtherMeans)+parseFloat(INVOICE.other_tender);
		$('.paid-row').show();
		$('.paid_total').text(format_cash(paid_total));
	}else{
		INVOICE.PaymentByOtherMeans = 0;
        if (INVOICE.other_tender !=0){
        paid_total=INVOICE.other_tender;
        $('.paid-row').show();
        $('.paid_total').text(format_cash(paid_total));}
        else{
		$('.paid-row').hide();
    ...
    if( !INVOICE.discount){
        $('.discount-row').show();
        $('.discount').text(format_cash(INVOICE.discount));
    }else{
        $('.discount-row').hide();
    }
	...
	INVOICE.total = INVOICE.total + tax_total;

	//add the totals to the view
	$('.sub_total').text(format_cash(sub_total));
    $('.discount').text(format_cash(INVOICE.discount));
	$('.tax_total').text(format_cash(tax_total));
    $('.other_tender').text(format_cash(INVOICE.other_tender));
	$('.grand_total').text(format_cash(INVOICE.total - INVOICE.other_tender));
...
function initializeTemplates(){
	...
	//init customer table code
	
	var tableCode = getParameterByName('table_code')
/*
	if(tableCode!=''){
		$('#customer_code_container').text(tableCode);
	}*/
...
//		LOAD['req_tableStatus'] = PARTNER.partner.req_tableStatus.replace('{merchant_id}', LOAD['partner_merchant_id']).replace('{pp_customer_id}', LOAD['pp_customer_id']);
		...
		LOAD['req_tableStatus'] = PARTNER.partner.req_tableStatus.replace('{location_id}', LOAD['pp_location_id'])
		.replace('{customer_id}', LOAD['pp_customer_id'])
		.replace('{tab_id}', LOAD['pp_tab_id']);
		
		
		//join friend api
		LOAD['req_tableJoinfriend'] = PARTNER.partner.req_tableJoinfriend.replace('{location_id}', LOAD['pp_location_id'])
		.replace('{my_tab_id}', LOAD['pp_tab_id']);
		
		//leave friend api
		LOAD['req_leaveFriend'] = PARTNER.partner.req_leaveFriend.replace('{location_id}', LOAD['pp_location_id'])
		.replace('{my_tab_id}', LOAD['pp_tab_id']);
		
		
		
		LOAD['req_tablePay'] = PARTNER.partner.req_tablePay;
...
function check_for_updates(){
	$.ajax(POLL['request'], {dataType: PARTNER.partner.req_dataType}).success(function(data){
		...
		var this_response = JSON.stringify(data);
        //alert("this response2 = "+this_response );
		...
		if(typeof data.merchant !== 'undefined'){
			...
			if(typeof data.table !== 'undefined'){ //This user has already been binded to a table
				...
				if(POLL['last_response'] != this_response){ //the invoice has changed or appeared
                    if (data.table.status!='2'){
					INVOICE = data.table;
					show_bill(INVOICE);
			...
			if(POLL['now']){ //do it again in the set interval
				POLL['last_response'] = this_response;
                //alert("POLL 2 = " +POLL['last_response']);
				POLL_INTERVAL = window.setTimeout(check_for_updates, POLL['interval']);
			}
...
function get_seat_split_total(){
	...
	grand_total = sub_total + tax_total- INVOICE.other_tender;
