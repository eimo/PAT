...
$(document).on('pageinit', '#split',  function(){
	...
	$('#split-slider').change(function(){
        //remove INVOICE.other_tender from INVOICE.total-INVOICE.other_tender
		var my_amount = (parseFloat($(this).val()) / 100) * (INVOICE.total);
...
$(document).on('pageinit', '#tip',  function(){
	...
	$('.tip-button').on(click_event, function(){
		...
		if(!SPLIT){		
			...
			var new_grand_total = parseFloat(tip_total) + parseFloat(INVOICE.total)  - parseFloat(INVOICE.PaymentByOtherMeans);
		}else{
			var tip_base = SPLIT;
			var tip_total = parseFloat((tip_base * (tip_percent/100)).toFixed(2));
